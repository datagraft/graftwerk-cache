...
node('swarm'){
	...
	try {
		...
		sh 'docker build -t datagraft/graftwerk-cache:latest .'		
	...
	if (env.BRANCH_NAME=="master") {
		...
		timeout (time:30, unit:'MINUTES') {		
			...
			sh 'docker tag datagraft/graftwerk-cache:latest datagraft/graftwerk-cache:$BUILD_NUMBER'
			sh 'docker push datagraft/graftwerk-cache:latest'
			sh 'docker push datagraft/graftwerk-cache:$BUILD_NUMBER'
			//Remove created image
			sh 'docker rmi datagraft/graftwerk-cache:latest'
		}	
	}
